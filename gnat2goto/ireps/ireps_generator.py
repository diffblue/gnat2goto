...
def write_raw(fd, txt):
    fd["content"].append({"kind": "text",
                          "text": str(txt).rstrip()})

def write(fd, txt):
    if len(txt.rstrip()) + 3*fd["indent"] < 80:
        write_raw(fd, txt)
    else:
        bs = []
        last_space = None
        for pos, c in enumerate(str(txt).rstrip()):
            actual_pos = 3 * fd["indent"] + pos
            if actual_pos < 80:
                if c == '(':
                    bs.append(pos)
                elif c == ')' and len(bs) > 0:
                ...
                break
        if len(bs) == 0:
            bs = copy(fd["bracket_stack"])
        a = txt[:last_space].rstrip()
        b = " " * (bs[-1] + 1) + txt[last_space:].strip()
        assert len(a) + fd["indent"] * 3 < 80
        assert len(b) + fd["indent"] * 3 < 80
        write_raw(fd, a)
        write_raw(fd, b)
    ...
    for pos, c in enumerate(str(txt.strip())):
        if c == '(':
            fd["bracket_stack"].append(pos)
        elif c == ")":
            del fd["bracket_stack"][-1]

def continuation(fd):
    assert fd["content"][-2]["kind"] == "text"
    assert fd["content"][-1]["kind"] == "text"
    len_before = len(fd["content"])
    # Merge the last two lines if it doesn't make them too long
    tmp = fd["content"][-2]["text"].rstrip() + " " + fd["content"][-1]["text"].strip()
    if 3 * fd["indent"] + len(tmp) < 80:
        del fd["content"][-1]
        fd["content"][-1]["text"] = tmp
    assert len(fd["content"]) in (len_before, len_before - 1)

def write_comment_block(fd, txt):
    write(fd, "-" * (len(txt) + 6))
    write(fd, "-- " + txt + " --")
    write(fd, "-" * (len(txt) + 6))
    write(fd, "")
...
def manual_indent(fd):
    fd["content"].append({"kind" : "indent"})
    fd["indent"] += 1


def manual_outdent(fd):
    fd["content"].append({"kind" : "outdent"})
    fd["indent"] -= 1
...
class IrepsGenerator(object):
        ...
        for sn in self.layout:
            for friendly_name in self.layout[sn]:
                ...
                if lo_kind in ("irep", "list"):
                    ...
                    # lo_typ == "bool" # TODO: remove or fix
                    a_kind = "bool"
            ...
            for sn in self.layout:
                for friendly_name in self.layout[sn]:
                    if friendly_name in optimal_lo:
                        ...
                        self.layout[sn][friendly_name] = \
                            (lo_typ, lo_idx, lo_kind)
    ...
    def all_classes(self):
        ...
        for sn, schema in self.schemata.iteritems():
            # source_location is unique case, hence the subclasses length check
            if not schema["used"] and len(schema["subclasses"]) != 0:
                if sn != "irep":
                    ar.append(sn)
    ...
    def make_class(self, root, fd):
        print "-- Generating class for %s" % root
    ...
    def rec2(self, sn, fd, prefix_len):  # TODO: rename function
        if self.schemata[sn]["used"]:
            write(fd, " " * prefix_len + self.schemata[sn]["ada_name"] + ",")
            self.top_sorted_sn.append(sn)
        for sc in sorted(self.schemata[sn]["subclasses"]):
            self.rec2(sc, fd, prefix_len)

    def rec3(self, kind, todo, groups):  # TODO: rename function
        ...
        for sc in self.schemata[kind]["subclasses"]:
            self.rec3(sc, todo, groups)

    def mk_precondition_in(self, param_name, kinds):
        todo = set(kinds)
        groups = []
        self.rec3("irep", todo, groups)
    ...
    def rec1(self, sn):  # TODO: rename function
        ...
        for i, sub in enumerate(schema.get("sub", [])):
            if "sub" in sub:
                ...
                self.register_sub_setter(sn, i, friendly_name,
                                         element_type, True, None)
        ...
        for fld in ("namedSub", "comment"):
            ...
            for friendly_name, data in schema.get(fld, {}).iteritems():
                if "constant" in data:
                    ...
                    self.register_named_setter(sn, "list",
    ...
    def emit_getter(self,
        ...
        for sn in sorted(i_kinds):
            layout_kind, layout_index, layout_typ = self.layout[sn][fn_name]
    ...
    def generate_code(self, optimize, schema_file_names):
        ...
        for setter_name in set(self.sub_setters) & set(self.named_setters):
            ...
            print "> sub in  :", ", ".join(
                set(list(self.sub_setters[setter_name].itervalues())[0]))
            print "> named in:", ", ".join(
                set(list(self.named_setters[setter_name].itervalues())[0]))
        ...
        for fn, category in [(self.emit_getter, "getters"),
            ...
            for fn_name in sorted(self.sub_setters):
                ...
                fn(fn_name    = fn_name,
                   fn_kind    = ("irep" if "value" in self.sub_setters[
                       fn_name] else "list"),
                   value_type = "irep",
        ...
        for (friendly_name, kinds) in self.sub_setters.iteritems():
            for (kind, schema_names) in kinds.iteritems():
                if kind == "list":
                    continue
                for (schema_name, (_, _, default_value)) in \
                    schema_names.iteritems():
                    if schema_name not in sub_setters_by_schema:
                        sub_setters_by_schema[schema_name] = []
                    sub_setters_by_schema[schema_name].append(
                        (friendly_name, default_value))

        for (friendly_name, kinds) in self.named_setters.iteritems():
            for (kind, schema_names) in kinds.iteritems():
                for (schema_name, (is_comment, actual_type, default_val)) in \
                        schema_names.iteritems():
                    ...
                    named_setters_by_schema[schema_name].append(
                        (friendly_name, param_type, default_val))
        ...
        for sn in self.top_sorted_sn:
            ...
            if sn in sub_setters_by_schema:
                for (friendly_name, default_value) in sub_setters_by_schema[sn]:
                    ...
                    formal_args.append((ada_casing(formal_name),
                                        ada_casing(friendly_name),
                                        "Irep",
                                        default_value))
            if sn in named_setters_by_schema:
                for (friendly_name, actual_type, default_value) in \
                        named_setters_by_schema[sn]:
                    formal_name = escape_reserved_words(friendly_name)
            ...
            arg_strings = ["%s : %s%s" %
                           (formal_name, formal_type, initialiser(
                               default_value, formal_type)) for
                        (formal_name, _, formal_type, default_value) in
                           formal_args]
            ...
            write(s, "return Irep;")
            with Indent(b):
                write(b, "Ret : constant Irep := New_Irep (%s);" %
                      schema["ada_name"])
            write(b, "begin")
            with Indent(b):
        ...
        write(s, "--  Debug procedure to print the given Irep "
              "to standard output")
        write(s, "")
